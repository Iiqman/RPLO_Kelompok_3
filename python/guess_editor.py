...
class GuessEditor:
    def __init__(self, root):
        self.root = root
        self.root.title("🎨 Guess Game Editor")
        ...
        self.json_path = self.base_path / "guess_data.json"
    ...
    def apply_theme(self):
        ...
        # Configure colors
        style.configure('TButton', background='#9B59B6', foreground='white', padding=6)
        style.map('TButton', background=[('active', '#8E44AD')])
    ...
    def load_data(self):
        ...
        try:
            if self.json_path.exists():
                ...
                print(f"[OK] Loaded {len(self.data)} questions from {self.json_path}")
            else:
                self.data = []
                self.save_data()
    ...
    def build_ui(self):
        ...
        self.correct_entry = tk.Entry(correct_frame, font=("Arial", 11), 
                                      relief=tk.SOLID, bd=1)
        self.correct_entry.pack(fill=tk.X, ipady=5)
    ...
    def on_question_select(self, event):
        ...
        self.correct_entry.insert(0, question.get("correct_answer", ""))
...
def main():
    print(f"\n{'='*60}")
    print(f"[*] Starting Guess Game Editor")
    print(f"{'='*60}\n")
    
    root = tk.Tk()
    app = GuessEditor(root)
